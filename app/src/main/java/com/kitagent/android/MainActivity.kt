...
class MainActivity : Activity() {
    ...
    private inner class CexTerminal(ctx:Activity):LinearLayout(ctx){
        ...
        private fun renderMetrics(x:CexApi.Market){if(childCount<2)return;val bar=getChildAt(1) as? LinearLayout?:return;val vals=listOf(String.format(Locale.US,"%.2f",x.last),String.format(Locale.US,"%.2f%%",x.change),String.format(Locale.US,"%.2f",x.fair),String.format(Locale.US,"%.2f",x.index),String.format(Locale.US,"%.4f%%",x.funding),String.format(Locale.US,"%.2f",x.volume));for(i in 0 until minOf(6,bar.childCount)){((bar.getChildAt(i) as? LinearLayout)?.getChildAt(1) as? TextView)?.text=vals[i]}}
    }
}
