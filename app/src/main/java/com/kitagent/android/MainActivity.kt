...
class MainActivity : Activity() {
    ...
    private fun buildShell() {
        val root = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setBackgroundColor(bg); setPadding(18, 10, 18, 0) }
        ...
        root.addView(scroll)
        nav = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER; setPadding(0, 5, 0, 4); setBackgroundColor(bg) }
    ...
    private fun rebuildNav() {
        ...
        sections.forEachIndexed { index, label ->
            val item = TextView(this).apply {
                text = navLabel(label); textSize = 9f; gravity = Gravity.CENTER; setTypeface(typeface, Typeface.BOLD)
                setTextColor(if (index == 0) cyan else muted); setPadding(2, 8, 2, 8); setOnClickListener { showSection(index) }
            }
            nav.addView(item, LinearLayout.LayoutParams(0, -1, 1f))
        }
    }

    private fun navLabel(label: String): String = when (label) { "Market Analysis" -> "MARKET"; "Chart Terminal" -> "CHART"; else -> label.uppercase() }

    private fun showSection(index: Int) {
        content.removeAllViews()
        if (index == 0) home() else {
            addHeader(sections[index])
            when (index) { 1 -> market(); 2 -> chart(); 3 -> cex(); 4 -> history(); 5 -> profile() }
        }
        updateNav(index)
        content.alpha = 0f
    ...
    private fun updateNav(active: Int) { for (i in 0 until nav.childCount) (nav.getChildAt(i) as TextView).setTextColor(if (i == active) cyan else muted) }

    private fun home() {
        val top = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(4, 18, 4, 12) }
        val brand = TextView(this).apply { text = "KITSETUPS"; textSize = 11f; letterSpacing = 0.25f; setTypeface(typeface, Typeface.BOLD); setTextColor(cyan) }
        top.addView(brand)
        val greeting = TextView(this).apply { text = "Your market.\nYour edge."; textSize = 34f; setTypeface(typeface, Typeface.BOLD); setTextColor(this@MainActivity.text); setPadding(0, 10, 0, 0) }
        top.addView(greeting)
        val sub = TextView(this).apply { text = "A live trading cockpit built for decisions, not decoration."; textSize = 13f; setTextColor(muted); setPadding(0, 8, 0, 0) }
        top.addView(sub)
        content.addView(top)
    ...
    private fun addHeroPanel(): View {
        val card = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(20, 20, 20, 20); setBackgroundResource(com.kitagent.android.R.drawable.bg_panel); layoutParams = LinearLayout.LayoutParams(-1, -2).apply { bottomMargin = 18 } }
        ...
        row.addView(live); card.addView(row)
    ...
    private fun animatePulse(view: View) {
        ...
        pulse.duration = 1100; pulse.repeatMode = ObjectAnimator.REVERSE; pulse.repeatCount = ObjectAnimator.INFINITE; pulse.start()
    }

    private fun addSectionLabel(label: String) { content.addView(TextView(this).apply { text = label; textSize = 9f; letterSpacing = 0.18f; setTypeface(typeface, Typeface.BOLD); setTextColor(muted); setPadding(4, 2, 4, 8) }) }
    ...
    private fun addActionGrid() {
        ...
        addAction(row1, "MARKET", "Scan setups"); addAction(row1, "CHART", "Open terminal"); actions.addView(row1)
        val row2 = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; setPadding(0, 8, 0, 0) }
        addAction(row2, "CEX", "Trade & connect"); addAction(row2, "HISTORY", "Review flow"); actions.addView(row2)
        content.addView(actions)
    }

    private fun addAction(row: LinearLayout, title: String, body: String) {
        val v = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(15, 15, 15, 15); setBackgroundResource(com.kitagent.android.R.drawable.bg_panel); setOnClickListener { performClickMotion(this) } }
    ...
    private fun performClickMotion(view: View) { view.animate().scaleX(0.97f).scaleY(0.97f).setDuration(80).withEndAction { view.animate().scaleX(1f).scaleY(1f).setDuration(140).start() }.start() }
